...
enum class SystemInfoType {
    ...
    /**
     * Gets encore related information. This parameter is not available on real systems,
     * but can be used by homebrew applications to get some emulator info.
     */
    ENCORE_INFORMATION = 0x20000,
};
...
/**
 * Accepted by svcGetSystemInfo param with ENCORE_INFORMATION type. Selects which information
 * to fetch from Encore. Some string params don't fit in 7 bytes, so they are split.
 */
enum class SystemInfoEncoreInformation {
    IS_ENCORE = 0,         // Always set the output to 1, signaling the app is running on Encore.
...
Result SVC::ControlMemory(u32* out_addr, u32 addr0, u32 addr1, u32 size, u32 operation,
    ...
    R_UNLESS((addr0 & Memory::ENCORE_PAGE_MASK) == 0, ResultMisalignedAddress);
    R_UNLESS((addr1 & Memory::ENCORE_PAGE_MASK) == 0, ResultMisalignedAddress);
    R_UNLESS((size & Memory::ENCORE_PAGE_MASK) == 0, ResultMisalignedSize);
...
Result SVC::CreateMemoryBlock(Handle* out_handle, u32 addr, u32 size, u32 my_permission,
                              u32 other_permission) {
    R_UNLESS(size % Memory::ENCORE_PAGE_SIZE == 0, ResultMisalignedSize);
...
Result SVC::GetSystemInfo(s64* out, u32 type, s32 param) {
    ...
    case SystemInfoType::ENCORE_INFORMATION:
        switch ((SystemInfoEncoreInformation)param) {
        case SystemInfoEncoreInformation::IS_ENCORE:
            *out = 1;
            break;
        case SystemInfoEncoreInformation::BUILD_NAME:
            CopyStringPart(reinterpret_cast<char*>(out), Common::g_build_name, 0, sizeof(s64));
            break;
        case SystemInfoEncoreInformation::BUILD_VERSION:
            CopyStringPart(reinterpret_cast<char*>(out), Common::g_build_version, 0, sizeof(s64));
            break;
        case SystemInfoEncoreInformation::BUILD_DATE_PART1:
            CopyStringPart(reinterpret_cast<char*>(out), Common::g_build_date,
                           (sizeof(s64) - 1) * 0, sizeof(s64));
            break;
        case SystemInfoEncoreInformation::BUILD_DATE_PART2:
            CopyStringPart(reinterpret_cast<char*>(out), Common::g_build_date,
                           (sizeof(s64) - 1) * 1, sizeof(s64));
            break;
        case SystemInfoEncoreInformation::BUILD_DATE_PART3:
            CopyStringPart(reinterpret_cast<char*>(out), Common::g_build_date,
                           (sizeof(s64) - 1) * 2, sizeof(s64));
            break;
        case SystemInfoEncoreInformation::BUILD_DATE_PART4:
            CopyStringPart(reinterpret_cast<char*>(out), Common::g_build_date,
                           (sizeof(s64) - 1) * 3, sizeof(s64));
            break;
        case SystemInfoEncoreInformation::BUILD_GIT_BRANCH_PART1:
            CopyStringPart(reinterpret_cast<char*>(out), Common::g_scm_branch,
                           (sizeof(s64) - 1) * 0, sizeof(s64));
            break;
        case SystemInfoEncoreInformation::BUILD_GIT_BRANCH_PART2:
            CopyStringPart(reinterpret_cast<char*>(out), Common::g_scm_branch,
                           (sizeof(s64) - 1) * 1, sizeof(s64));
            break;
        case SystemInfoEncoreInformation::BUILD_GIT_DESCRIPTION_PART1:
            CopyStringPart(reinterpret_cast<char*>(out), Common::g_scm_desc, (sizeof(s64) - 1) * 0,
                           sizeof(s64));
            break;
        case SystemInfoEncoreInformation::BUILD_GIT_DESCRIPTION_PART2:
            CopyStringPart(reinterpret_cast<char*>(out), Common::g_scm_desc, (sizeof(s64) - 1) * 1,
                           sizeof(s64));
            break;
        default:
            LOG_ERROR(Kernel_SVC, "unknown GetSystemInfo encore info param={}", param);
            *out = 0;
            break;
        }
        break;
...
Result SVC::GetProcessInfo(s64* out, Handle process_handle, u32 type) {
    ...
    case ProcessInfoType::PRIVATE_SHARED_SUPERVISOR_HANDLE_USED_MEMORY:
        ...
        *out = process->memory_used;
        if (*out % Memory::ENCORE_PAGE_SIZE != 0) {
...
Result SVC::MapProcessMemoryEx(Handle dst_process_handle, u32 dst_address,
    ...
    if (size & 0xFFF) {
        size = (size & ~0xFFF) + Memory::ENCORE_PAGE_SIZE;
    }
