...
CitraContext::CitraContext(ConfigCallbackInterface& config_interface,
...
std::optional<std::string> CitraContext::LoadROM(const std::string& rom_path) {
    window->MakeCurrent();
...
void CitraContext::RunFrame() {
    window->MakeCurrent();
    window->RunFrame();
    audio_resampler->Flush();
}

void CitraContext::Reset() {
    window->MakeCurrent();
    system.Reset();
}

std::pair<u32, u32> CitraContext::GetVideoDimensions() const {
    return window->GetVideoDimensions();
}

u32 CitraContext::GetGLTexture() const {
...
void CitraContext::ReadFrameBuffer(u32* dest_buffer) {
    window->MakeCurrent();
    return window->ReadFrameBuffer(dest_buffer);
}

std::span<const s16> CitraContext::GetAudio() const {
    return audio_resampler->GetAudio();
}

void CitraContext::ReloadConfig() const {
    config->Reload();
    window->ReloadConfig(config->GetWindowScaleFactor());
}

std::size_t CitraContext::StartSaveState() {
    window->MakeCurrent();
    return savestate_mt->StartSaveState();
}

void CitraContext::FinishSaveState(void* dest_buffer) {
